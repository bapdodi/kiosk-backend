...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    Page<Product> findAllByDeletedAtIsNullOrderBySortOrderAscIdAsc(Pageable pageable);
    ...
    @Query(value = "SELECT DISTINCT p FROM Product p JOIN p.categories c WHERE p.deletedAt IS NULL AND c.mainCategory = :mainCategory",
            countQuery = "SELECT COUNT(DISTINCT p) FROM Product p JOIN p.categories c WHERE p.deletedAt IS NULL AND c.mainCategory = :mainCategory")
    ...
    /** (대분류 + 중분류) 쌍에 소속된 상품. */
    @Query(value = "SELECT DISTINCT p FROM Product p JOIN p.categories c WHERE p.deletedAt IS NULL AND c.mainCategory = :mainCategory AND c.subCategory = :subCategory",
            countQuery = "SELECT COUNT(DISTINCT p) FROM Product p JOIN p.categories c WHERE p.deletedAt IS NULL AND c.mainCategory = :mainCategory AND c.subCategory = :subCategory")
