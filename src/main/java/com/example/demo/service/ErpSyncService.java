...
public class ErpSyncService {
     ...
     * 기다리지 않도록 상품 동기화 때 같이 받아 둔다. ERP 에서 사라진 거래처는 지우지 않는다 —
    ...
    public int syncCustomers() {
        ...
        for (Map<String, Object> row : rows) {
            ...
            customers.add(Customer.builder()
                    ...
                    .danga(row.get("DANGA") == null ? null : toInteger(row.get("DANGA")))
                    .syncedAt(now)
                    .build());
        }
        ...
        for (com.example.demo.entity.OrderItem item : order.getItems()) {
                ...
                try {
                    ...
                    if (danga == 2 && itemOutA > 0)
                        ...
                        // 단가표를 못 고른 경우다. actualPrice 는 소비자가(OUTC) 그대로 나가므로
                        // 조용히 넘어가면 과다 청구가 된다. DANGA=1(매입 거래처)은 애초에 주문 목록에서
